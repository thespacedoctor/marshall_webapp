...
class test_views_transients_akas(BaseTest):
    ...
    def test_01_views_transients_akas_post(self):
        ...
        respsonse = self.testapp.post(
            "/transients/akas", params=params, status=404)
        self.assertEqual(respsonse.status_code, 404)

    def test_views_transients_akas_get(self):
        # PARAM DICTIONARY = URL TOKENS
        params = {}
    ...
    def test_views_transients_akas_delete(self):
        # PARAM DICTIONARY = URL TOKENS
        params = {}
        respsonse = self.testapp.delete('/transients/akas',
                                        params=params, status=404)
        print(respsonse)
        self.assertEqual(respsonse.status_code, 404)

    def test_views_transients_akas_put(self):
        # PARAM DICTIONARY = URL TOKENS
        params = {}
        respsonse = self.testapp.put('/transients/akas',
                                     params=params, status=404)
        print(respsonse)
        self.assertEqual(respsonse.status_code, 404)
