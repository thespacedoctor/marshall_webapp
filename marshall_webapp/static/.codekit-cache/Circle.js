...
Circle = (function() {
    ...
    Circle = function(centerRaDec, radiusDegrees, options) {
        ...
        this.fillOpacity = options['fillOpacity'] || undefined;
    ...
    Circle.prototype.draw = function(ctx, projection, frame, width, height, largestDim, zoomFactor) {
        ...
        if (this.color) {
            ...
            if (this.fillOpacity) {
                ctx.fillStyle = ColorOpacity(this.color, this.fillOpacity);
            }
        }
        ctx.beginPath();
...
function ColorOpacity(hex, op) {
    ...
    rgba = "rgba(" + result["r"] + "," + result["g"] + "," + result["b"] + "," + op + ")";
    return rgba;
}
