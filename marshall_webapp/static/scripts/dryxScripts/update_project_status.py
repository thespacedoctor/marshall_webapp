...
========================
:Summary:
    Update the project status of a MD page
...
def main():
    ...
    # RECONSTRUCT URL
    if "REQUEST_URI" not in os.environ.keys():
        thisUrl = "command-line"
    else:
        thisUrl = os.environ['REQUEST_URI']

    if "none" not in projectStatus.lower():
        projectStatus = u'"%(projectStatus)s"' % locals()
    else:
        projectStatus = "null"

    # update the project status
    sqlQuery = u"""
        update helper_pages_projects set projectStatus = %(projectStatus)s where pageId = %(pageId)s 
    """ % locals()
