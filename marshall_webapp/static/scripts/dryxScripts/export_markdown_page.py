...
# encoding: utf-8
"""
export_markdown_page.py
=======================
:Summary:
    Export a MD page to a folder in dropbox to be edited on any device
...
def main():
    ...
    pathToAssetsImages = settings["path to assets directory"]
    if "/" == pathToAssetsImages[-1]:
        pathToAssetsImages = pathToAssetsImages[:-1]
    pathToAssetsImages = pathToAssetsImages + "/images"
    notes = settings["path to dropbox notes"]
    websiteName = settings["website name"]
    prefix = settings["assets prefix"]
    privacyLevel = "personal"

    mdw.mysqltools.export_md_page_to_textfile.export_md_page_to_textfile(
        dbConn=dbConn,
        log=log,
        pageId=pageId,
        pathToExportFolder=notes,
        pathToAssetsImages=pathToAssetsImages,
