...
==================
:Summary:
    convert a sqlquery to downloadable file or viewable webpage
...
def main():
    ...
    if "csvType" not in locals():
        csvType = "human"
    if "csvTitle" not in locals():
        csvTitle = "Exported from PESSTO Marshall"
    if "csvFilename" not in locals():
        csvFilename = "export.csv"
    if "returnFormat" not in locals():
        returnFormat = "webpageView"
    ...
    csvOutput = dct.sqlquery_to_csv_file.sqlquery_to_csv_file(
        ...
        returnFormat=returnFormat  # plainText | webpageDownload | webpageView
    )

    print csvOutput

    ################ >ACTION(S) ###############
    # x-pessto-marshall-webpage-template
    # x-print-as-json-to-cgi
    # redirect-with-cgi
    # dbConn.commit()
    # dbConn.close()
    return
