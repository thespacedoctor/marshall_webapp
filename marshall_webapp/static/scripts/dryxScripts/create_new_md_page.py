...
# encoding: utf-8
"""
create_new_md_page.py
=====================
:Summary:
    Add an external alias to the MarkdownWiki
...
def main():
    ...
    newId = int(rows[0]["pageId"]) + 1

    ################ > VARIABLE SETTINGS ######
    # SETUP THE VARIOUS URL TOKEN VALUES
    sqlQuery = """INSERT INTO base_md_pages (
            title,
            content,
            pageId
        )
    ...
    # add an entry in to the projects table
    sqlQuery = u"""
        insert ignore into helper_pages_projects(pageId) select distinct pageId from base_md_pages;
    """ % locals()
    dms.execute_mysql_write_query(
        sqlQuery=sqlQuery,
        dbConn=dbConn,
    ...
    redirectURL = "%(redirectURL)s%(newId)s" % locals()
