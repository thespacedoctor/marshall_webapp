...
View = (function() {
    ...
    createListeners = function(view) {
        ...
        $(view.reticleCanvas).bind("mouseup mouseout touchend", function(e) {
            ...
            if (objs) {
                // alert(JSON.stringify(objs[0]));
                var o = objs[0];
                var count = 1;
                while (!(o.marker) && (objs.length >= count)) {
                    var o = objs[count];
                    count++;
                }
                // display marker
                ...
                */
                if (!view.dragging && !view.mode == View.SELECT) {
                    ...
                    if (closest) {
                        ...
                        var o = closest[0];
                        var count = 1;
                        while (!(o.marker) && (closest.length >= count)) {
                            var o = closest[count];
                            lastHoveredObject = closest[count];
                            count++;
                        }
                        // display marker
                        if (o.marker) {
                            ...
                            if (lastHoveredObject && lastHoveredObject.marker) {
                                view.popup.setSource(lastHoveredObject);
                                view.popup.hide();
                            }
                        }
