...
Aladin = (function() {
    ...
    var Aladin = function(aladinDiv, requestedOptions) {
        ...
        var locationDiv = $('<div class="aladin-location">' + (options.showFrame ? '<select class="aladin-frameChoice"><option value="' + CooFrameEnum.J2000 + '" ' + (frameInJ2000 ? 'selected="selected"' : '') + '>J2000</option><option value="' + CooFrameEnum.GAL + '" ' + (!frameInJ2000 ? 'selected="selected"' : '') + '>GAL</option></select>' : '') + '<span class="aladin-location-text"></span></div>')
            .appendTo(aladinDiv);
        ...
        if (options.showFullscreenControl) {
            $('<div class="aladin-fullscreenControl aladin-maximize" title="Full screen"><i class="icon-resize-enlarge"></i></div>')
                .appendTo(aladinDiv);
        }
    ...
    Aladin.prototype.toggleFullscreen = function() {
        ...
        if (isInFullscreen) {
            this.fullScreenBtn.html('<i class="icon-resize-shrink"></i>');
        } else {
            this.fullScreenBtn.html('<i class="icon-resize-enlarge"></i>');
        }
...
Aladin.prototype.displayFITS = function(url, options) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            aladin.setOverlayImageLayer(aladin.createImageSurvey(label, label, response.data.url, "equatorial", response.data.meta.max_norder, {
                imgFormat: 'png'
            }));
